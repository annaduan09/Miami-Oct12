...
#NEAREST NEIGHBOR (some are used for testing, to determine feature buffer distances)
 ...
 houses <-
   ...
   mutate(
     ...
     busNN1 = nn_function(st_c(st_centroid(houses)), st_c(bus), 5),
     #culture
     cultureNN1 = nn_function(st_c(st_centroid(houses)), st_c(culture), 1),
     cultureNN1 = nn_function(st_c(st_centroid(houses)), st_c(culture), 5),
     #food/drinks
...
```{r Designing Regression}
 ...
 #what is bldg? Bldg,
 #AD: not sure but Ken said we can't use
```
...
reg.training <-
  lm(SalePrice ~ ., data = st_drop_geometry(miami.training) %>%
                             dplyr::select(SalePrice, ActualSqFt, LotSize, Zoning, Stories.cat, Bath.cat, Pool, medHHInc, Dock, Bed.cat, middleCatch, age, monthhousingcost, Patio, foodEstablishments, timeToWork,parkArea, floodInsureType))
...
miami.test <-
  ...
  mutate(Regression = "Baseline Regression",
         ...
         SalePrice.APE = SalePrice.AbsError / SalePrice.Predict)%>% #0.05589877
...
kfold.MLR = function(fit,k=10,data=fit$model) {       #AD: I'm not sure what this is for?
  sum.sqerr = rep(0,k)
  sum.abserr = rep(0,k)
  sum.pererr = rep(0,k)
  y = fit$model[,1]
...
#AD: not sure what this is
#BC: I searched on Google and I guess this may work
miami.res = resid(miami.test.nhood)

plot(miami.test.nhood$SalePrice.Predict, miami.res, 
     ylab="Residuals", xlab="SalePrice", 
     main="Residual Map") 
abline(0, 0)  

#or we can do this:
par(mfrow = c(2, 2))
plot(reg.cv$finalModel)
...
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(miami.training) %>%
                                 dplyr::select(neighborhood, SalePrice, ActualSqFt, LotSize, Zoning, Stories.cat, Bath.cat, Pool, medHHInc, Dock, Bed.cat, middleCatch, age, monthhousingcost, Patio, foodEstablishments, timeToWork,parkArea, floodInsureType))
summary(reg.nhood)
...
#Baseline Regression	366345.6	0.0494198
#Neighborhood Effects	365363.5	-1.3121554
...
housesPredictions <-                   
  houses %>%
  mutate(prediction = predict(reg.cv, houses),
         team_name = 'Panda')
...
write.csv(predictions, "PANDA.csv")

#AD: "The column names MUST to be "prediction", "Folio", and "team_name"" - piazza
...
# AD: they said to do facetted plot, so I changed it a bit
```
...
st_join(bothRegressions, acsRaceIncome) %>%
  ...
  kable(caption = "MAPE by neighborhood racial context")
...
st_join(bothRegressions, acsRaceIncome) %>%
  ...
  kable(caption = "MAPE by neighborhood income context")

#Test set MAPE by neighborhood racial context
#Regression	Majority Non-White	Majority White
#Baseline Regression	-44%	35%
#Neighborhood Effects	-71%	-150%
...
#Baseline Regression	55%	-10%
#Neighborhood Effects	-220%	-70%
```
