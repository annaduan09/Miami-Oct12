...
stargazer(as.data.frame(housesSub), type="html", digits=1, title="Table 1: Descriptive Statistics for Miami Houses", out = "Miami Data.txt")
...
ggplot(data = housesKnown, aes(x = distWater, y = SalePrice)) +
  ...
  labs(title = "Figure 2.1: Distance to Shoreline and Sale Price", subtitle = "Miami and Miami Beach, FL", x="Distance to water (mile)", y="Sale price ($)") +
...
ggplot(data = housesKnown, aes(x = parkArea, y = SalePrice)) +
  geom_point(size=2, shape=20) +
  labs(title = "Figure 2.2: Nearby Park Space and Sale Price", subtitle = "Miami and Miami Beach, FL", x="Area of park (squared mile)", y="Sale price ($)") +
  geom_smooth(method = "lm", se=F, colour = "green") +
    plotTheme()
...
ggplot(data = housesKnown, aes(x = pctVacant, y = SalePrice)) +
  geom_point(size=2, shape=20) +
 labs(title = "Figure 2.3: Share of Vacant Properties and Sale Price", subtitle = "Miami and Miami Beach, FL", x="Vacant rate (%)", y="Sale price ($)") +
  geom_smooth(method = "lm", se=F, colour = "green") +
    plotTheme()
...
ggplot(data = housesKnown, aes(x = busStops, y = SalePrice)) +
  geom_point(size=2, shape=20) +
 labs(title = "Figure 2.4: Bus Stops and Sale Price", subtitle = "Miami and Miami Beach, FL", x="Number of bus stops", y="Sale price ($)") +
  geom_smooth(method = "lm", se=F, colour = "green") +
    plotTheme()
...
houses_hood <- houses %>% st_join(., nhoods, join = st_within)
...
stargazer(reg.training, type="html", digits=1, title="Table 2: LM of Training Data", out = "Training LM.txt")
```
...
ggplot(data = miami.test) +
  ...
  labs(title = "Figure 5.1: Observed Sale Price and Absolute Error", x="Sale price ($)", y="Absolute Error ($)") +
  plotTheme()

ggplot(data = miami.test) +
  geom_point(aes(x = SalePrice, y = SalePrice.APE)) +
  labs(title = "Figure 5.2: Observed Sale Price and Absolute Percent Error", x="Sale price ($)", y="APE (%)") +
  plotTheme()
...
ggplot(reg.cv$resample, aes(x=MAE)) +
  geom_histogram() +
  labs(title = "Figure 6.1: Mean Average Error in Cross Validation Tests", X="MAE ($)", y="number of test sets") +
  plotTheme()
...
plot(housesKnown$SalePrice, housesKnown$lagPrice, xlab="Sale Price ($)", ylab="Spatial Lag Price ($)", main="Figure 6.2: The Spatial Lag of Housing Price")
...
miami.test %>%                
  ...
  labs(title = "Figure 8.1: Spatial Lag of Price Errors", x="Lag price error ($)", y="Sale price ($)") +
...
housesKnown_hood <- housesKnown %>% st_join(., nhoods, join = st_within)
inTrain_hood <- createDataPartition(
              y = paste(housesKnown$Zoning, housesKnown$floodInsureType, housesKnown$neighborhood),
              p = .60, list = FALSE)
miami.training_h <- housesKnown[inTrain_hood,]
miami.test_h <- housesKnown[-inTrain_hood,] 
miami.training_h <- miami.training_h %>% st_join(., nhoods, join = st_within)
miami.test_h <- miami.test_h %>% st_join(., nhoods, join = st_within)

#Make new neighborhood regression
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(miami.training_h) %>% 
...
miami.test.nhood <-
  ...
  mutate(Regression = "Neighborhood Effects",
         SalePrice.Predict = predict(reg.nhood, miami.test_h), #613237.3
...
miami.test <- miami.test %>% st_join(., nhoods, join = st_within)
#Check accuracy
bothRegressions <-
  rbind(
