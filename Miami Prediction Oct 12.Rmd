...
```{r Fig 3: 4 SCATTERPLOT home price correlation}
...
```{r Fig 4: MAP Sale Price, echo=FALSE}
...
ggplot() +
  ...
  labs(title = "Figure 5.1: Nearby Park Area", subtitle = "Miami, FL") +
  mapTheme()

ggplot() +
  geom_sf(data = acs, fill = "gray90", colour = "white") +
  geom_sf(data = water, fill = "light blue", colour = "light blue") +
  geom_sf(data = housesKnown, aes(colour = floodInsureType)) +
...
hist(miami.test$SalePrice.AbsError)
hist(miami.test$SalePrice.APE)
hist(miami.test$SalePrice.Predict)

ggplot(data = miami.test)+geom_point(aes(x = SalePrice, y = SalePrice.AbsError))
...
miami.test %>%                
  mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)) %>%  #AD: fixed it
...
#Baseline Regression	351280.6	0.7101703
#Neighborhood Effects	354142.8	0.7047983
