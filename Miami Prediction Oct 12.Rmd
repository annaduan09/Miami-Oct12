...
houses <-
 ...
 houses <- 
  ...
  mutate(distWater.cat = case_when(
                  distWater < 0.25  ~ "Near shore",
                  distWater >= 0.25                    ~ "Not near shore"))
...
```{Fig 5: MAP 3 Independent Variables}
...
drop <- c("x","TOD")
houses = houses[,!(names(houses) %in% drop)] #AD: what is this?
...
ggplot(reg.cv$resample, aes(x=MAE)) +  #should this be average error for each fold? not working properly..
  geom_histogram() +
  plotTheme()
...
#10.1: map of your residuals for your test set
...
#10.3: Plot: spatial lag in errors

  #AD: I'm not super clear on this section

miami.test %>% 
  ggplot(aes(SalePrice.Error, SalePrice)) +       #what's this for? Can you write some notes here so I know how to explain?
  geom_point() +
...
left_join(
  ...
  mutate(miami.test, predict.fe = 
                        predict(lm(SalePrice ~ neighborhood, data = miami.test), 
                        miami.test)) %>%
    st_drop_geometry %>%
    group_by(neighborhood) %>%
...
housesPredictions <-                   
  housesUnknown %>%
  mutate(SalePrice = predict(reg.nhood, housesUnknown,
         TeamName = Panda)) #AD: what do you think!:)
...
```{r Fig 11: map of predicted values}
#11 Provide a map of your predicted values for where ‘toPredict’ is both 0 and 1.
# facetted plot: one side with hosues with given SalePRice values, other site with our predicted values
```

```{r Fig 12: Using the test set predictions, provide a map of MAPE by neighborhood}
...
```{r Fig 13: Scatterplot of MAPE by neighborhood as a function of mean price by neighborhood}

```

## Does this model work equally for different demographic groups?
```{r Fig 14: race and income}
#RACE & INCOME
  #make new layer
acsRaceIncome <- 
  acs %>%
