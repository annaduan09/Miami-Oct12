...
inTrain <- createDataPartition(
              y = paste(houses$Zoning, houses$floodInsureType, houses$NAME.y), 
              p = .60, list = FALSE)
...
miami.test <-
  ...
  mutate(Regression = "Baseline Regression",
         SalePrice.Predict = predict(reg.training, miami.test), #611056.3
         SalePrice.Error = SalePrice.Predict - SalePrice, #106261.4
         SalePrice.AbsError = abs(SalePrice.Predict - SalePrice), #488903.4
         SalePrice.APE = (abs(SalePrice.Predict - SalePrice)) / SalePrice.Predict)%>% #-0.8543894
...
miami.test.nhood <-
  ...
  mutate(Regression = "Neighborhood Effects",
         ...
         SalePrice.Error = SalePrice - SalePrice.Predict, # -108442.4      #Dont need these three lines for the final predictions
...
bothRegressions <- 
  rbind(
    dplyr::select(miami.test, starts_with("SalePrice"), Regression, NAME.y) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    dplyr::select(miami.test.nhood, starts_with("SalePrice"), Regression, NAME.y) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))   
...
#Baseline Regression	488903.4	-0.8543894
#Neighborhood Effects	491238.7	0.7109973
...
#BC: THEY ARE SO DIFFERENT, MAYBE WE SHOULD ASK THIS IN THE OFFICE HOUR
...
```{r testing figures}
####MAP: DEPENDENT VAR. (sale price)#### BC: what is this for?
ggplot() +
...
ggplot(data = housesKnown, aes(x = parkArea, y = SalePrice)) +
  geom_point(size=2, shape=20) +
  geom_smooth(method = "lm", se=F, colour = "green")
  #elem catch #BC: how to read this map?
ggplot(data = housesKnown, aes(x = elemCatch, y = SalePrice)) +
  geom_point(size=2, shape=20)
  #bus stops - interesting that houses with more nearby cost less - maybe because they're near the street #BC:maybe, or maybe the distribution of bus stops are intended to be in those poor neighborhood/depressed area?
