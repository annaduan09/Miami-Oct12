...
predictions <-                      #AD: why are there so many rows? we should only have 876
  miami.test %>%
...
predictedPrices <- miami.test %>% dplyr::select(SalePrice.Predict, Folio) %>%
  st_drop_geometry()
predictedHouses <- #so that we can map all the sale prices, including predicted
  houses %>% filter(toPredict == 1) %>%
  dplyr::select(-SalePrice) %>%
  left_join(predictedPrices)
```
