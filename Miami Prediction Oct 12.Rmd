...
#NEAREST NEIGHBOR (some are used for testing, to determine feature buffer distances)
 ...
 houses <-
   ...
   mutate(
     #contaminated
      contamNN1 = nn_function(st_c(st_centroid(houses)), st_c(contaminated), 1),
     contamNN2 = nn_function(st_c(st_centroid(houses)), st_c(contaminated), 2),
     contamNN3 = nn_function(st_c(st_centroid(houses)), st_c(contaminated), 3),
     contamNN4 = nn_function(st_c(st_centroid(houses)), st_c(contaminated), 4),
     contamNN5 = nn_function(st_c(st_centroid(houses)), st_c(contaminated), 5),
...
houses <-
 ...
 houses <- 
  ...
  mutate(distWater.cat = case_when(
                  ...
                  distWater >= 1.05 & distWater < 1.5  ~ "1 - 1.5 Miles",
                  distWater >= 1.5                    ~ "1.5+ Miles"))
...
houses <- 
  houses %>%
  mutate(contaminated.cat = case_when(
                  contaminated == 1  ~ "Contam Zone",
                  contaminated == 0  ~ "Non Contam"))
...
#AD:, timeToWork, medHHInc, monthhousingcost, pctVacant, pctWhite, pctRenterOcc, busStops, metrorailStops, cultureSpots, parkArea, commercialProperties, distWater, LivingSqFt, Bath, Bed, Mailing.Zip
 ...
 cor.test(houses$floodInsureType, houses$SalePrice, method = "pearson")
